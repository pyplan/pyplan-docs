...
- Updates
  ...
  - Develop HTML components aligned with HTMLFilter, including styles for KPIs and various input types.
...
- Fixed Issues
  - Fixed an issue where users with the app open during a UI deploy encountered a ChunkLoadError when navigating to a lazy-loaded route.
  - Fixed an issue where the app failed to open when its CPU architecture was ARM and the assigned resources were x86.
  - Fixed an issue where attempting to copy a node as values resulted in a KeyError.
  - Fixed an issue where clicking a link to navigate to a node from a DynamicHtml component resulted in a blank diagram due to an invalid codeLayoutId.
...
- Updates
  ...
  - Adjust workflow emails to allow language changes, include company logos, enhance design, add overdue dates, and inform users of blocking tasks.
  - Enable drag and drop for pinned nodes and allow repositioning of pinned nodes.
  - Add functionality to reorder open tabs using drag and drop.
- Fixed Issues
...
- Fixed Issues
  - The droppable area does not function correctly when pivoting in tables.
  - Styles are not applied to the first column of a table when the index hide option is used.
  - Date type selectors do not respect minimum and maximum dates when opened from a node in the diagram.
  - Minor fixes.
